...
class Variational_Ansatz:
    ...
    """
    def __init__(self, _H, _G, _ref, _params, _fci=None, _k=1):
        """
        _H      : sparse matrix
    ...
    def overlap_callback(self,x):
        try:
            err = np.sqrt(np.vdot(self.der, self.der))
            print(" Iter:%4i Overlap with FCI state = %20.16f Gradient Norm %10.1e Gradient Max %10.1e" %(self.iter,
                self.curr_overlap, err, np.max(np.abs(self.der))))
        except:
            print(" Iter:%4i Current Energy = %20.16f Psi Norm Error %10.1e" %(self.iter,
                self.curr_overlap, 1-self.psi_norm))
        self.iter += 1
        self.overlap_per_iteration.append(self.curr_overlap)
        sys.stdout.flush()
...
class tUCCSD(Variational_Ansatz):
    ...
    def overlap(self, params):
        new_state = self.prepare_state(params)
        assert(new_state.transpose().conj().dot(new_state).toarray()[0][0]-1<0.0000001)
        overlap = np.vdot(self.fci.transpose().conj(), new_state)

        self.curr_overlap = scipy.sparse.find(np.absolute(overlap))[2]
        return -self.curr_overlap
    ...
    def prepare_state(self,parameters):
        ...
        for n in range(self.k):
            for k in reversed(range(0, len(parameters))):
                new_state = scipy.sparse.linalg.expm_multiply((parameters[k]*self.G[k]/float(self.k)), new_state)
    ...
    def overlap_gradient(self,parameters):
        """ 
        """
        grad = []
        new_ket = self.prepare_state(parameters)
        hbra = self.fci.transpose().conj()

        term = 0
        ket = cp.deepcopy(new_ket)
        grad = self.overlap_recurse(parameters, grad, hbra, ket, term)
        self.der = grad
        return np.asarray(grad)
    ...
    def overlap_recurse(self, parameters, grad, hbra, ket, term):
        if term == 0:
            hbra = hbra
            ket = ket
        else:
            hbra = (scipy.sparse.linalg.expm_multiply(-self.G[term-1]*parameters[term-1], hbra.transpose().conj())).transpose().conj()
            ket = scipy.sparse.linalg.expm_multiply(-self.G[term-1]*parameters[term-1], ket)
        grad.append(hbra.dot(self.G[term]).dot(ket).toarray()[0][0].real)

        if term<len(parameters)-1:
            term += 1
            self.overlap_recurse(parameters, grad, hbra, ket, term)
        return np.asarray(grad)
